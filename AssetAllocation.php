...
<body>
	...
	<nav id="navbar">
		<ul>
			...
			<li><a href="./Calculator.php">報酬試算</a></li>
		</ul>
	...
	<div class="block">
    	...
    	<form method="post" action="AssetAllocationOutput.php" name="AAO" onsubmit="return chk();">
        <div class="keyIn">
          股票名稱/代碼搜尋：
          <select multiple name="selectedStock[]" id="stockinfo" data-placeholder="輸入選取欲選擇類股" style="width:90%;" class="chosen-select" tabindex="6">
            ...
            for($i=1;$i<=30;$i++)
            {
               $cat = "SELECT name FROM category WHERE number=' ".$i." ' ";
               $st= $link->query($cat);
               $show=$st->fetch();
               echo "<optgroup label= ".$show['name']." ><br/> ";  //查詢類別名稱
              
               $sql = "SELECT stockName,new FROM stockInfo WHERE categoryNumber=' ".$i." '";
               $sth= $link->query($sql);  //查詢該類別個股
               While($row =$sth->fetch())
                  {
                      echo "<option>".$row['new']." ".$row['stockName']."</option><br/>"; 
                  } 				
            }
			$link = null; //結束與資料庫連線
            ?>
        ...
        <div class="cart" id="cart">
            ...
            <ul class="list"></ul>
            <div id="option">
                資本：<input name="assetInto" type="text" id="assetInto">元<br>
                預期報酬：<input name="reward" type="text" id="reward">%<br>
